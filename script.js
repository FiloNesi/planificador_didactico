...
const ui = {
    ...
    showMultiSelectModal(type) {
        ...
        options.forEach(opt => {
            ...
            cb.type = 'checkbox';
            cb.id = `ms-${type}-${opt.replace(/\s+/g, '-')}`; // Create a safe ID
            cb.value = opt;
    ...
    renderInfoSummary() {
        ...
        const title = (data.situacionAprendizaje || '').split('\n')[0] || 'Sesión sin título';
    ...
    buildForm() {
        ...
        infoSummary.id = 'info-summary';
        infoSummary.className = 'hidden mb-4';
        form.appendChild(infoSummary);
        ...
        const fieldsets = [
            ...
            { step:2, legend: 'Detalles de la Sesión', fields:[
                { id:'contextoReto', label:'Contexto-reto', type:'textarea' }, { id:'contenidos', label:'Contenidos', type:'textarea' }, { id:'metodologias', label:'Metodologías', type:'textarea' }, { id:'resultadosEsperados', label:'Resultados esperados al final de la sesión', type:'textarea' }, { id:'preparacionPrevia', label:'Preparación previa', type:'textarea' },
            ]},
            ...
            { step:5, legend: 'Marco Pedagógico (Automático)', fields:[
                { id:'competenciaEspecifica', label:'Competencia Específica', type:'readonly-textarea' }, { id:'descriptoresOperativos', label:'Descriptores Operativos', type:'readonly-textarea' }, { id:'competenciasClave', label:'Competencias Clave', type:'readonly-textarea' }, { id:'contenidosTransversales', label:'Contenidos Transversales', type:'textarea' },
            ]},
        ...
        form.addEventListener('input', app.triggerAutoSave);
    },
    ...
    renderSessionList(sessionsToRender = state.allSessions) {
        ...
        // Sort the incoming list of sessions to ensure we always work with the most recent ones first.
        sessionsToRender.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

        // Determine if we are showing the default list (all sessions) or a filtered list from a search.
        const isDefaultView = sessionsToRender === state.allSessions;

        // If it's the default view, only take the 5 most recent sessions.
        // If it's a search result, show all matching sessions.
        const sessionsToDisplay = isDefaultView ? sessionsToRender.slice(0, 5) : sessionsToRender;
    ...
    printReport() {
        ...
        const createText = (content) => {
            ...
            return !strContent || strContent.trim() === ''
                ? '<p class="text-slate-500 italic mt-2">No especificado</p>'
                : `<div class="prose prose-sm max-w-none mt-2" style="white-space: pre-wrap;">${strContent}</div>`;
        };
        ...
        const situacionTitle = (data.situacionAprendizaje || '').split('\n')[0] || 'Situación de Aprendizaje sin Título';
        const situacionBody = createText(data.situacionAprendizaje);

        const actividadesContent = [data.actividad1, data.actividad2, data.actividad3, data.actividad4].map((act, i) => act && act.trim() !== '' ? `<div><strong class="font-semibold text-slate-700">Actividad ${i + 1}:</strong>${createText(act)}</div>` : '').filter(Boolean).join('<hr class="my-4 border-slate-200">');
...
const app = {
    ...
    getFormData() {
        ...
        const allFormFields = [
            'fecha', 'competenciaEspecifica', 'descriptoresOperativos', 'competenciasClave',
            'situacionAprendizaje', 'contextoReto', 'contenidos', 'metodologias', 
            'resultadosEsperados', 'preparacionPrevia', 'actividad1', 'actividad2', 
            'actividad3', 'actividad4', 'instrumentosEvaluacion', 'recursos', 
            'observaciones', 'adaptacion', 'criteriosCalificacion'
        ];
        const multiSelectFields = ['materia', 'curso', 'sesionNum', 'tiposEvaluacion', 'contenidosTransversales', 'unidadDidactica'];
    ...
    duplicateSession() {
        ...
        const situacionField = document.getElementById('situacionAprendizaje');
        situacionField.value = `[COPIA] ${situacionField.value}`;
        
        const title = (s.situacionAprendizaje || '').split('\n')[0] || 'Sin título';
